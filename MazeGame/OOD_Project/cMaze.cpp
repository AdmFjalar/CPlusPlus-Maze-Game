...
/// <summary>
/// An edited version of the included print function. Used to insert entities in their corresponding positions in the maze.
/// </summary>
void cMaze::defineCharacters()
{
	m_strings.clear();
	...
	for (int r = 0; r < m_dimensions.y; r++)
	{
		...
		{
			if (r == player.coordinates.y && c == player.coordinates.x) //Checks if the print loop is in the player's coordinates.
			{
				p_chars[r][c] = 'O'; //Inserts the player's corresponding character, O, in the char array. 
			}
			else if (enemy != nullptr && r == enemy->coordinates.y && c == enemy->coordinates.x) //Checks if the print loop is in the enemy's coordinates.
			{
				p_chars[r][c] = 'Q'; //Inserts the enemy's corresponding character, Q, in the char array.
			}
			else if (coll1 != nullptr && r == coll1->coordinates.y && c == coll1->coordinates.x) //Checks if the print loop is in the first collectible's coordinates.
			{
				p_chars[r][c] = coll1->displayCharacter; //Inserts the first collectible's corresponding character in the char array.
			}
			else if (coll2 != nullptr && r == coll2->coordinates.y && c == coll2->coordinates.x) //Checks if the print loop is in the second collectible's coordinates.
			{
				p_chars[r][c] = coll2->displayCharacter; //Inserts the second collectible's corresponding character in the char array.
			}
			else { //If no entity is present on the current coordinates then the corresponding maze character (i.e. wall, floor or endpoint) is inserted in the char array.
...
/// <summary>
/// Tries to move the player to the new coordinates and also handles collisions with enemy and collectibles.
/// </summary>
/// <param name="xCoor"></param>
/// <param name="yCoor"></param>
void cMaze::movePlayer(int xCoor, int yCoor)
{
	if (enemy != nullptr && player.coordinates.x == enemy->coordinates.x && player.coordinates.y == enemy->coordinates.y) //Checks if player is colliding with the enemy.
	{
		player.lives--;
	}

	if (p_Cells[yCoor][xCoor].display != getWallChar()) //Checks if the new position is legal (i.e. if it is not inside a wall).
	{
		player.coordinates.x = xCoor;
		player.coordinates.y = yCoor;

		if (player.coordinates.x == m_exit.x && player.coordinates.y == m_exit.y) //Checks if the player has reached the exit point.
		{
			print();
			player.hasWon = true;
		}
		else if (coll1 != nullptr && player.coordinates.x == coll1->coordinates.x && player.coordinates.y == coll1->coordinates.y) //Checks if player is colliding with collectible 1.
		{
			score += coll1->value;
			delete coll1;
		}
		else if (coll2 != nullptr && player.coordinates.x == coll2->coordinates.x && player.coordinates.y == coll2->coordinates.y) //Checks if player is colliding with collectible 2.
		{
			score += coll2->value;
			delete coll2;
		}
...
/// <summary>
/// Tries to move enemy back and forth vertically.
/// </summary>
void cMaze::moveEnemy()
{
		if (p_Cells[enemy->coordinates.y - 1][enemy->coordinates.x].display != getWallChar() && !enemy->changedDirection) //Checks if the new position is legal and if the enemy has not changed direction.
		{
			enemy->Move(cVector2(enemy->coordinates.x, enemy->coordinates.y - 1));
		}
		else if (p_Cells[enemy->coordinates.y + 1][enemy->coordinates.x].display != getWallChar() && enemy->changedDirection) //Checks if the new position is legal and if the enemy has changed direction.
		{
			enemy->Move(cVector2(enemy->coordinates.x, enemy->coordinates.y + 1));
		}

		if (p_Cells[enemy->coordinates.y - 1][enemy->coordinates.x].display == getWallChar() && !enemy->changedDirection) //Checks if the enemy has reached the wall and inverses it's direction if that's the case.
		{
			enemy->changedDirection = true;
		}
		else if (p_Cells[enemy->coordinates.y + 1][enemy->coordinates.x].display == getWallChar() && enemy->changedDirection) //Checks if the enemy has reached the wall and inverses it's direction if that's the case.
		{
...
cMaze::cMaze()
{
	coll1 = new Collectible();
	coll2 = new Collectible(50, 'G');
	enemy = new Enemy;
}
...
void    cMaze::generateMaze()
{
	...
	m_start = randPos;
	player.coordinates = m_start;
	m_endpoints.push_back(randPos);

	coll1->coordinates = cVector2(SIGNED_MASK & (2 * m_mtRand() + 1) % (m_dimensions.x - 1), SIGNED_MASK & (2 * m_mtRand() + 1) % (m_dimensions.y - 1));
	coll2->coordinates = cVector2(SIGNED_MASK & (2 * m_mtRand() + 1) % (m_dimensions.x - 1), SIGNED_MASK & (2 * m_mtRand() + 1) % (m_dimensions.y - 1));
	
	enemy->coordinates = cVector2(SIGNED_MASK & (2 * m_mtRand() + 1) % (m_dimensions.x - 1), SIGNED_MASK & (2 * m_mtRand() + 1) % (m_dimensions.y - 1));
...
void        cMaze::printCells()
{
	...
	for (int r = 0; r < m_dimensions.y; r++)
	{
		...
		for (int c = 0; c < m_dimensions.x; c++)
		{
			if (player.coordinates.x == r && player.coordinates.y == c)
			{
				std::cout << "O";
			}
			else
